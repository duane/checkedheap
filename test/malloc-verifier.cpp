...
public:
  ...
  void run (unsigned long iterations = numIterations) {
	...
	for (int j = 0; j < numToFree; j++) {
      ...
      if (r % 2 == 0) {
	for (unsigned j = 0; j < rng.next() % (MAX_OBJECTS / MAX_FULL_FRACTION); j++) {
	  mallocOne();
	}
      } else {
	freeOne();
...
private:
...
#ifdef __APPLE__
  objectSize[index]  = malloc_good_size(malloc_size(objectMap[index]));
#else
  ...
  void fill (unsigned long index) {
    ...
    for (long i = 0; i < (long)(sz / sizeof(unsigned long)); i++) {
      ptr[i] = fillValue + i;
    }
  }
